...
function UpdatePhone() {
  ...
  function validateField(name: string, value: string) {
    ...
    if (newPhone.length > 0 && 
      ...
      if (newPhone !== confirmPhone) {
        const message = 'Phone number inputs do not match';
        addClassToInformUser(name, 'invalid', message);
      } else {
        addClassToInformUser('new_phone', 'valid', ' ');
        addClassToInformUser('confirm_new_phone', 'valid', ' ');
      }
      return;
    }
  ...
  async function generateOtp(resend = false) {
    setIsLoading(true);
      ...
      try {
        const res = await axios.post('/api/profile/otp/generate/phone', {
          phone: newPhone,
        });
  ...
  async function verifyOtp() {
    ...
    try {
      const res = await axios.post('/api/profile/otp/verify/phone', {
        phone: newPhone,
        otp: otpInput.join('')
      });
        ...
        <div className={`${styles.update_email_container} ${styles.step_container}`}>
          ...
          <div className={styles.form_control}>
            <input
              ...
              value={newPhone}
              onChange={(e) => {
                resetInputElement('new_phone');
                setNewPhone(e.target.value);
              }}
              onBlur={(e) => validateField('new_phone', e.target.value)}
            />
            <span data-name='new_phone' className={styles.input_error_message}></span>
          </div>
          ...
          <div className={styles.form_control}>
            <input
              ...
              value={confirmPhone}
              onChange={(e) => {
                resetInputElement('confirm_new_phone');
                setConfirmPhone(e.target.value);
              }}
              onBlur={(e) => validateField('confirm_new_phone', e.target.value)}
            />
            <span data-name='confirm_new_phone' className={styles.input_error_message}></span>
          </div>
